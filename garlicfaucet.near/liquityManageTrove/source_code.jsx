/**
 * @description
 * 5 out of all contracts are being used for manage trove.
 ...
 *   2. Get ETH:USD price.
 *   3. Get your ICR.
 *   4. Get your colletral.
 *   5. Get your debt.
 *   6. get liquity system called TCR(total collateral ratio).
 */
const infoHandler = () => {
  Ethers.provider()
    .getNetwork()
 ...
 * Check List
 *   1. balance check.
 *   2. normal mode(TCR >= 1.5) : ICR must be greater than 110%.
 *   3. recovery mode(TCR < 1.5) : ICR must be greater than 150%.
 *   4. recvoery mode(TCR < 1.5) : new ICR must be greater than old ICR.
 */
const checkFunc = () => {
  ...
  if (state.balance && state.option === "deposit" && state.token === "ETH") {
    const amount = ethers.utils.parseUnits(
      state.value.toString() || "0",
      "ether"
    );
    if (amount.gt(state.balance)) {
      State.update({ check: false });
      return;
    }
  }
  ...
  if (
    ...
    (tcr < 150 && Number(state.currentICR) > state.updatedICR)
  ) {
    State.update({ check: false });
    return;
  }
  if (state.updatedDebt && state.updatedDebt < 2000) {
    State.update({ check: false });
    return;
  }
 ...
 * @description
 * change handler.
 * If input change, update UI and run checkFunc(validate function).
 *   1. deposit ethereum
 *   2. withdraw ethereum
 *   3. deposit(repay) LUSD
 *   4. withdraw LUSD
 */
      ...
      .then((raw) => {
        ...
        State.update({
          ...
          updatedICR:
            Math.round(
              ((Number(state.currentColl) /
                (Number(state.currentDebt) + Number(value) + estimatedFee)) *
                Number(state.currentPrice) +
                EPSILON) *
                1000
            ) / 1000,
...
const tokenHandler = (token) => {
  State.update({ token: token, value: "" });
 ...
 * Call transaction user click Confirm button.
 *   1. BorrowerOperations.addColl
 *   2. BorrowerOperations.withdrawColl
 *   3. BorrowerOperations.repayLUSD
 *   4. BorrowerOperations.withdrawLUSD
 */
const confirmHandler = () => {
  ...
  const coll = ethers.BigNumber.from(
    ethers.utils.parseEther((state.updatedColl || state.currentColl).toString())
  );

  const debt = ethers.BigNumber.from(
    ethers.utils.parseEther((state.updatedDebt || state.currentDebt).toString())
  );
 ...
 * 4. Enter a value greater than 0 in input.
 * 5. If all stats displayed on the screen UI are normal(green) and the balance is sufficient, confirm available.
 */
return (
  <ManageWrapper>
