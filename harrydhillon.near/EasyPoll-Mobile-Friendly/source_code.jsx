...
const fVToken = viewFVTokens();

State.init({
  displaying: fVToken ? 0 : 3,
  hoveringElement: "",
...
const renderAbortPollCreationModal = () => {
  return (
    <Widget
      src="harrydhillon.near/widget/Easypoll-Abort-Poll-Creation-Modal"
      props={{
        state: state,
        closeModalClickingOnTransparent: closeModalClickingOnTransparent,
        tabs: tabs,
...
return (
  ...
  >
      ...
      <div style={{ width: 200 }} className="p-2">
        <div>
          ...
          <p style={{ margin: "0", fontWeight: "bold", fontSize: "0.9rem" }}>
            {fVToken ? "Verified Human" : "Non-Verified Human"}
          </p>
        </div>
      </div>
    </HeaderContainer>
    ...
    ) : fVToken && state.displaying == tabs.NEW_POLL.id ? (
      <div
        className="px-4"
        style={{
          backgroundColor: "white",
