...
const PAGODA_CONSOLE_BACKEND_URL = "https://d7d4-94-204-65-133.ngrok-free.app";
const PAGODA_CONSOLE_FRONTEND_URL = "http://localhost:3000";
...
`;
console.log(state.repositorySlug.test, "<<< repoSLug");
return (
  <Wrapper>
    <Header size={size}>
      <Thumbnail size={size}>
    ...
    <Actions>
      ...
      <Button
        disabled={state.creatingDeploy || state.repositorySlug}
        onClick={() => {
          State.update({
            creatingDeploy: true,
          });
          return asyncFetch(
            `${PAGODA_CONSOLE_BACKEND_URL}/deploys/addConsoleDeployProject`,
            {
              body: `{ "projectName": "${
                context.accountId
              }_${widgetName}_${Date.now()}", "nearSocialComponentPath": "${src}" }`,
              headers: {
            ...
            State.update({
              repositorySlug: res.body.repositorySlug,
              creatingDeploy: false,
            })
          );
        }}
    ...
    {state.repositorySlug ? (
      <div class="alert alert-success" role="alert">
        <a
          href={`${PAGODA_CONSOLE_FRONTEND_URL}/deploys?repositorySlug=${state.repositorySlug}`}
          target="_blank"
        >
          Deployed to console — check it out!
        </a>
      </div>
    ) : null}
  </Wrapper>
