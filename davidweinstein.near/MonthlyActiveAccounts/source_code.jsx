...
let data = fetch(
  "https://api.flipsidecrypto.com/api/v2/queries/a6c8964e-2c61-4ec5-82d0-818931643d25/data/latest",
  {
    subscribe: true,
    method: "GET",
    headers: {
...
const METRIC_NAME = "New Monthly Activations";
...
const height = 500;
const maxValue =
  Math.max(...data.body.map((d) => d.NEW_ACTIVATED_WALLETS)) * 1.1;
                ...
                {data.body.map((d, i) => (
                  <g key={i} className="barTextH">
                    <rect
                      ...
                      y={height - (d.NEW_ACTIVATED_WALLETS / maxValue) * height}
                      width={width / data.body.length - 2}
                      height={(d.NEW_ACTIVATED_WALLETS / maxValue) * height}
                      fill="#61dafb"
                    />
                    <text
                    ...
                    <text
                      ...
                      textAnchor="middle"
                    >
                      {formatNumberWithCommas(d.NEW_ACTIVATED_WALLETS)}
                    </text>
                  </g>
                ))}
              </svg>
