...
const getTokenBalance = (receiver) => {
  ...
  return erc20.balanceOf(Ethers.provider().getSigner()).then((rawBalance) => {
...
const getVote = (_signer) => {
  getTokenBalance(_signer).then((_balance) => {
    State.update({ balance: _balance });
  });
  const erc20 = new ethers.Contract(
    voterTokenContract,
    tokenAbi,
...
if (state.balance === undefined && state.sender) {
  ...
  erc20.balanceOf(Ethers.provider().getSigner()).then((_balance) => {
    State.update({
      balance: _balance,
    });
  });
