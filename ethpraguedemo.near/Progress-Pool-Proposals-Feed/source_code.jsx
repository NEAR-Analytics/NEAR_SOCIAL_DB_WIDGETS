...
if (state.proposalNumber !== undefined && state.proposalNumber > 0) {
  const _pulled_proposals = state.pulled_proposals;
  ...
  for (let num = 0; num < state.proposalNumber; num++) {
    proposals.proposals(num).then((_proposal) => {
      _pulled_proposals = [..._pulled_proposals, { num, result }];
      console.log(_proposal);
    });
  }
  State.update({ pulled_proposals: _pulled_proposals });
}
    ...
    {state.pulled_proposals.map((item) => (
      <div class="row">
        <div class="col-sm-1 col-2 d-flex">
          <Widget src="ethpraguedemo.near/widget/Progress-Pool-Question-Button-Upvote" />
        </div>
        <div class="col-10">
          <div class="row">
            <div class="col">
              <div class="row">
                <div class="col">
                  <H2>
                    <a href={item.result[0]}>{item.result[0]}</a>
