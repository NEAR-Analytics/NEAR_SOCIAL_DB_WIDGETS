...
function draw() {
  ...
  ctx.beginPath();
  ctx.arc(75, 75, 52.5, 0, Math.PI * 2, false); // Earth orbit
  ctx.stroke();
...
const animate = () => {
  setTimeout(function() {
      data.canvasImage = canvas.toDataURL("image/png").replace("image/png", "image/octet-stream")
    sendState();
    draw()
...
return (
  <div
    class="d-flex bg-black justify-content-center"
    style={{
      height: 100,
    }}
  >
    <img src={state.canvasImage} class="w-auto h-auto" />
    <iframe
      srcDoc={srcData}
      onMessage={(data) => {
        State.update({ ...data });
